#!/usr/bin/env bash
# Builds the recsys-serving Lambda zip for x86_64 py3.12. boto3 and its dependencies come
# from the Lambda runtime, so they are left out (a ~3 MB zip instead of ~20 MB).
